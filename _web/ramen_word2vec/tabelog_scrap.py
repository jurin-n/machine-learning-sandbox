...
class Tabelog:
    ...
    def scrape_item(self, item_url, mode):
        ...
        time.sleep(1)
        if r.status_code != requests.codes.ok:
            print('error:not found{ item_url }')
            return
    ...
    def scrape_review(self, review_url):
        ...
        time.sleep(1)
        if r.status_code != requests.codes.ok:
            print('error:not found{ review_url }')
            return False
    ...
    def get_review_text(self, review_detail_url):
        ...
        time.sleep(1)
        if r.status_code != requests.codes.ok:
            print('error:not found{ review_detail_url }')
            return
...
tokyo_ramen_review = Tabelog(base_url="https://tabelog.com/tokyo/rstLst/ramen/",test_mode=False, p_ward='東京都内', begin_page=1, end_page=30)
#CSV保存
